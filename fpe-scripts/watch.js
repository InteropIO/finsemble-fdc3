...
access(seedDirectory, constants.F_OK | constants.W_OK, (err) => {
	if (err) {
		errorLog(`${seedDirectory} ${err.code === "ENOENT" ? "does not exist" : "is read-only"}`);
	} else {
...
async function updateConfig(seedDirectory, currentFile) {
	...
	try {
		...
		const deletedConfigValues = oldProjectImportConfigValues.filter(
			(configValue) => !projectConfig.importConfig.includes(configValue)
		);
