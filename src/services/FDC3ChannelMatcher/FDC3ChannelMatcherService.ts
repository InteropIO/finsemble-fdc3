...
class FDC3ChannelMatcher extends Finsemble.baseService {
	...
	onExternalProviderStoreUpdate(thirdPartyProvider: Provider) {
		const updateProviderChannel =
			async (providerName: string, channelName: string, channelValues: ProviderChannel) => {
				try {
					const { inbound = null, outbound = null } = channelValues;
					const { inboundListener, outboundListener } =
						await this.setFDC3ProviderChannel(providerName, channelName, channelValues, this.providerState);
		...
		providers.forEach(([providerName, providerChannels]) => {
			// find the channel(s) that have updated / changed and 
			// then update the fdc3channel and state
			...
			// TODO: When FDC3 adds a method to remove channels,
			// add the ability to remove channel listeners
		});
	}
	...
	async setFDC3ProviderChannel(providerName: string,
        ...
        /**
         * Set the channel that listens to fdc3 context via receivingChannel
         * and send it on via broadcastingChannel
         */
		async function setFDC3ChannelCommunication(
			{ receivingChannelName,
				broadcastingChannelName,
				receivingChannelListener,
			}:
				{
					receivingChannelName: string,
					broadcastingChannelName: string,
					receivingChannelListener: Listener,
				},
		): Promise<Listener | null> {
			try {
				// check for channels that are not set
				// this can be possible as inbound / outbound can be null (not supported)
				if (receivingChannelName && broadcastingChannelName) {
					if (receivingChannelListener) {
						receivingChannelListener.unsubscribe();
					}
					...
					const listener = await receivingChannel.addContextListener(
						async (context: Context) => {
							console.group();
							console.log(`receivingChannel: ${receivingChannelName} && broadcastingChannel: ${broadcastingChannelName} --- context: `);
