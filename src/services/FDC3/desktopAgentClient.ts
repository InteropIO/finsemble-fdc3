...
export default class DesktopAgentClient extends EventEmitter
	...
	constructor( strict: Boolean, Finsemble?: typeof FSBL) {
    super();
    this.#strict = strict;
    this.#FSBL = Finsemble;
  }
  ...
  async open(name: string, context?: Context) {
    ...
    const { err, response } = (await this.#FSBL.Clients.LauncherClient.spawn(
      name,
      {
        data: {
          fdc3: { context },
          linker:
            this.#currentChannel.id !== "global"
  ...
  addContextListener(
    ...
    this.#log(
      "DesktopAgentClient: addContextListener",
  ...
  async raiseIntent(intent: string, context: Context, target?: string) {
    ...
    this.#FSBL.Clients.Logger.log(
      "DesktopAgent.raiseIntent response: ",
      response.data
    );
    return response.data;
  }
  ...
  addIntentListener(intent: string, handler: ContextHandler): Listener {
    ...
    this.#FSBL.Clients.RouterClient.addListener(
      `FDC3.intent.${intent}`,
      routerHandler
    );
    return {
      unsubscribe: () => {
        this.#FSBL.Clients.RouterClient.removeListener(
          `FDC3.intent.${intent}`,
          routerHandler
        );
      },
  ...
  async joinChannel(channelId: string) {
    ...
    if (this.#channelChanging) {
      throw new Error(
        "Currently in process of changing channels. Rejecting this request. " +
          channelId
      );
    }
    ...
    if (channelId !== "global") {
      this.#FSBL.Clients.LinkerClient.linkToChannel(
        channel.id,
