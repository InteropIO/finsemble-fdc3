  ...
  createRouterEndpoints() {
    ...
    for (const ApiCall of DesktopAgentApiList) {
      RouterClient.addResponder(
        `FDC3.DesktopAgent.${ApiCall}`,
        async (err: Error, queryMessage: any) => {
          ...
          this.desktopAgent.windowName = (queryMessage.header
            .origin as string).replace("RouterClient.", "");

          try {
            let response;
            switch (ApiCall) {
              case "addContextListener":
                response = await this.desktopAgent.addContextListener(
                  queryMessage.data.contextType,
                  () => {
                    // TODO
                  }
                );
              ...
              case "addIntentListener":
                response = await this.desktopAgent.addIntentListener(
                  queryMessage.data.intent,
                  () => {
                    // TODO
                  }
                );
                break;
              case "broadcast":
                response = await this.desktopAgent.broadcast(
                  queryMessage.data.context
                );
                break;
              case "findIntent":
                response = await this.desktopAgent.findIntent(
                  queryMessage.data.intent,
                  queryMessage.data.context
                );
                break;
              case "findIntentsByContext":
                response = await this.desktopAgent.findIntentsByContext(
                  queryMessage.data.context
                );
                break;
              case "getCurrentChannel":
                response = await this.desktopAgent.getOrCreateChannel(
                  queryMessage.data.channelId
                );
                break;
              ...
              case "getSystemChannels":
                ...
                break;
              case "joinChannel":
                response = await this.desktopAgent.joinChannel(
                  queryMessage.data.channelId
                );
                break;
              case "leaveCurrentChannel":
                ...
                break;
              case "open":
                response = await this.desktopAgent.open(
                  queryMessage.data.name,
                  queryMessage.data.context
                );
                break;
              case "raiseIntent":
                response = await this.desktopAgent.raiseIntent(
                  queryMessage.data.intent,
                  queryMessage.data.context
                );
                break;
      ...
      RouterClient.addResponder(
        ...
        async (err: Error, queryMessage: any) => {
          try {
            ...
            switch (ApiCall) {
              case "addContextListener":
                response = await channel.addContextListener(
                  queryMessage.data.contextType,
                  () => {
                    // TODO
                  }
              ...
              case "getCurrentContext":
                response = await channel.getCurrentContext(
                  queryMessage.data.contextType
                );
                break;
            }
