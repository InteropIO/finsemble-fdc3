...
import Logger from "@chartiq/finsemble/dist/types/clients/logger";
...
class FDC3Client {
  ...
  constructor(Finsemble?: typeof FSBL) {
    ...
    const setupAgents = async () => {
      ...
      const validLinkerChannels = linkerState.channels.map(
      ...
      let linkerChannels = Object.keys(
        this.#FSBL.Clients.LinkerClient.channels
      );
      ...
      const channelsToRemove = linkerChannels.filter(
        (channel) => !validLinkerChannels.includes(channel)
      );

      // Unlink from everything the component should not be a member of
      for (const channel of channelsToRemove) {
        this.#FSBL.Clients.LinkerClient.unlinkFromChannel(
          channel,
          this.#FSBL.Clients.WindowClient.getWindowIdentifier()
        );
      }
      ...
      const updateAgents = async (err: any, response: any) => {
        ...
        if (this.#strict) {
          ...
          if (
            linkerChannels.length === 1 &&
            currentChannel &&
            linkerChannels[0] === currentChannel.id
          )
            return;

          // remove current channel
          if (currentChannel) {
            linkerChannels = linkerChannels.filter(
          ...
          const linkerChannels = Object.keys(
            this.#FSBL.Clients.LinkerClient.channels
          );
          const desktopAgentChannels = Object.keys(this.desktopAgentsByChannel);
          const desktopAgentsToRemove = desktopAgentChannels.filter(
            (channel) => !linkerChannels.includes(channel)
          );
      ...
      this.#FSBL.Clients.LinkerClient.onStateChange(
        async (err: any, data: any) => {
          await updateAgents(err, data);
        }
      );
    };
    setupAgents();
  ...
  async getOrCreateDesktopAgent(channel: string): Promise<DesktopAgent> {
    try {
      ...
      const desktopAgent = new DesktopAgent(this.#strict, this.#FSBL);
  ...
  async getSystemChannels() {
    ...
    const {
      err,
      response,
    } = await this.#FSBL.Clients.RouterClient.query(
