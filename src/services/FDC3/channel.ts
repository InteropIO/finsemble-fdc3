...
export default class C implements Channel {
  ...
  broadcast(context: Context): void {
    ...
    if (this.id === "global") {
      ...
      this.#FSBL.Clients.RouterClient.transmit(
        `FDC3.broadcast.${(context as any).type}`,
        context
      );
  ...
  async getCurrentContext(contextType?: string): Promise<Context | null> {
    if (contextType) return this.contexts[contextType];
    if (!contextType) return this.currentContext || null;
  }
