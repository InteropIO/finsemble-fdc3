	...
	return (
		<div className="api-example">
			...
			<InteractiveDemo
				...
				setInputValuesForSnippet={setInputValuesForSnippet}
				inputs={inputs}
			/>
			...
			<CodeSection
				snippet={
					typeof snippet === "string"
						? snippet
						: snippet(
								...inputValuesForSnippet.map(({ inputValue }) =>
									inputValue.includes("{") ? inputValue : JSON.stringify(inputValue)
								)
						  )
				}
				result={apiResult}
...
function CodeSection(props: CodeSectionProps) {
	...
	return (
		<div>
			<span className="snippet-buttons" onClick={() => setShowCodeSnippet(!showCodeSnippet)}>
				{!showCodeSnippet ? "show code snippet  ➕" : "hide code snippet ➖"}
			</span>
			<span
				className={`snippet-buttons${result ? "" : "-disabled"}`}
				onClick={() => result && setShowCodeResult(!showCodeResult)}
			>
				{!showCodeResult ? "show code result ➕" : "hide code result ➖"}
			</span>
...
function CopyToClipboard({ content }: { content: any }) {
	...
	useEffect(() => {
		const timer = setTimeout(() => setClipboardCopyMessageVisible(false), 5000);
		return () => {
			clearTimeout(timer);
		};
	...
	return (
		<>
			<span onClick={copyToClipboard} className="copy-code">
				<CopyIcon />
			</span>
